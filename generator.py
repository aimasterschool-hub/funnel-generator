...
def _generate_text_content(
    ...
    system = [
        {
            "type": "text",
            "text": (
                "あなたは日本語のダイレクトレスポンス・コピーライターです。\n\n"
                "【厳守ルール】\n"
                "- テキストはすべて以下の情報源の内容・登場人物・数字・商品名に基づいて生成すること\n"
                "- 情報源に登場しない固有名詞・数字・人物名は絶対に使わないこと\n"
                "- 各要素に「上限○文字」と書かれている場合は必ずその文字数以内に収めること\n\n"
                "【LPコピーライティング原則（必ず守ること）】\n"
                "- 情報源の文章をそのまま要約・転記しない。読者の感情を動かすLP表現に書き直すこと\n"
                "- 一文を短く切る。改行を使って縦に読ませるリズムを作ること\n"
                "- 数字・価格・固有名詞は正確に使う（「高額ツール」ではなく「30万円のツール」）\n"
                "- 「あなた」への直接語りかけ・断言表現を使う（「変えられます」より「変わります」）\n"
                "- 価格対比・社会的証明を積極的に使う（「通常30万円→今だけ0円」「3,000名が使う」）\n"
                "- 強い動詞・インパクトのある言葉を選ぶ（「強制チャージ」「確定」「叩き出す」）\n"
                "- 読者の恐怖・欲求・緊急性に直接訴えること\n\n"
                "【時制・状態に関する絶対ルール】\n"
                "- このLPは「今まさに申し込みできる状態」のページである\n"
                "- 情報源に「明日〇時募集開始」「〇月〇日から販売」などの未来形・開始予告の表現があっても絶対に使わないこと\n"
                "- 締め切り・枠数の緊急性は「残りわずか」「締め切り間近」「今すぐ」など現在進行形の表現に置き換えること\n"
                "- 「募集開始」→「申込受付中」「今すぐ申し込む」に変換すること\n"
                "- 「明日〇時」「〇月〇日」など特定の日付・時刻は使わないこと（設定情報に限定枠数・締め切りが明示されている場合はその内容のみ使う）\n\n"
                f"【情報源（販売者情報・商品情報・台本・骨子）】\n{script}"
            ),
            "cache_control": {"type": "ephemeral"},
        }
    ]
    ...
    if style_reference:
        system.append({
            "type": "text",
            "text": (
                "【参考ファネル（構成・文章変換パターンの手本）】\n"
                "以下は同じ台本から作られた高品質なLPファネルの実例です。\n"
                "次の2点だけを徹底的に学んでください：\n"
                "①「台本のどの情報を、どのセクションにどう配置しているか」という構成の作り方\n"
                "②「台本の生の文章を、どのようなLPコピーに変換しているか」という言葉の変換パターン\n\n"
                "【無視すること】\n"
                "- 色・背景・デザイン・レイアウトに関する記述はすべて無視する\n"
                "- 参考例の数字・名前・商品名はそのまま使わず、情報源の内容に置き換えること\n\n"
                f"{style_reference[:12000]}"
            ),
        })
...
def _generate_image_brief(
    ...
    user_prompt = (
        ...
        f"【画像の用途】{purpose}\n"
        f"【サイズ仕様】{size_hint}\n"
        f"【制作コンテキスト（background生成の参考にするが、このメモをそのまま出力しないこと）】\n{notes}\n\n"
    ...
    try:
        ...
        brief_data = {
            ...
            "size": "",
            "background": "ダーク・プレミアムな雰囲気。黒〜ネイビー系背景にゴールドのアクセント",
            "subject": None,
            "copy_text": pre_copy,
            "font_spec": None,
            "notes": "",
