...
## 登録訴求の種別
以下のいずれかを記載し、その根拠も書く：
- 無料プレゼントあり:（LINE登録で無料配布するオプト特典の名称・内容）
- 情報公開型:（無料プレゼントではなく「情報・手法・ノウハウを受け取る」訴求の場合。例: 1万円を58万円にした手法を公開）
...
※オプト特典（LINE登録だけで受け取れるもの）と購入者特典（商品購入者向け）を必ず区別して記載すること

## 緊急性・限定性
（配布終了条件・人数制限・期間制限の具体的な根拠）

## 各エピソードCTA
（動画ファネルの場合のみ記載。該当しない場合は「なし」）
- ep1の質問文:（例: 1000万円稼げたら何に使いたいですか？）
